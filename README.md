...
# gotfix/postgresql:9.6.2-5

> Alternatively the image is available from [`quay.io/gotfix/postgresql:9.6.2-5`](https://quay.io/repository/gotfix/postgresql)
...
```bash
docker pull gotfix/postgresql:9.6.2-5
```
...
```bash
docker run --name postgresql -itd --restart always \
  gotfix/postgresql:9.6.2-5 -c log_connections=on
```
...
```bash
docker run --name postgresql -itd --restart always \
  gotfix/postgresql:9.6.2-5 -c logging_collector=on
```
