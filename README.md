...
Automated builds of the image are available on [Dockerhub](https://hub.docker.com/r/phpbcom/postgresql) and is the recommended method of installation.

```bash
docker pull phpbcom/postgresql:9.6.2-1
```

Alternatively you can build the image yourself.

```bash
docker build -t phpbcom/postgresql github.com/phpbcom/docker-postgresql
```
...
```bash
docker run --name postgresql -itd --restart always \
  phpbcom/postgresql:9.6.2-1 -c log_connections=on
```
...
```bash
docker run --name postgresql -itd --restart always \
  phpbcom/postgresql:9.6.2-1 -c logging_collector=on
```
