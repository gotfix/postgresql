...
> Alternatively the image is available from [`quay.io/gotfix/postgresql:9.6.2-3`](https://quay.io/repository/gotfix/postgresql)
...
```bash
docker run --name postgresql -itd --restart always \
  gotfix/postgresql:9.6.2-3 -c log_connections=on
```
...
```bash
docker run --name postgresql -itd --restart always \
  gotfix/postgresql:9.6.2-3 -c logging_collector=on
```
